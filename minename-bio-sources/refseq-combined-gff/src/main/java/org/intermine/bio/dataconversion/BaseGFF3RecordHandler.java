...
public class BaseGFF3RecordHandler extends GFF3RecordHandler
{
    ...
    protected Map<String,Item> createdSOtermMap = new HashMap<String,Item>();
     ...
     * Get SO term item
     */
    protected Item getSOTerm(String featureType) {
        Item soTerm = createdSOtermMap.get(featureType);
        if (soTerm == null) {
            soTerm = converter.createItem("SOTerm");
            try {
                converter.store(soTerm);
            } catch (Exception e) {
                System.out.println("Exception while storing SO Term:" + soTerm + "\n" + e);
            }
     ...
     * Set feature SO term
     */
    protected void setFeatureSOTerm(String soTerm) {
        Item feature = getFeature();
        feature.setReference("sequenceOntologyTerm", getSOTerm(soTerm));
    }
